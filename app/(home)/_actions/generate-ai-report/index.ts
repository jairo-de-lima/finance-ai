...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  //   // Gerar o hash das transações atuais
  //   const currentTransactionsHash = generateTransactionsHash(transactions);

  //   // Verificar se já existe um relatório para o usuário e mês
  //   const existingReport = await db.aiReport.findFirst({
  //     where: {
  //       userId,
  //       month,
  //     },
  ...
  //   // Comparar os hashes das transações
  //   if (existingReport && existingReport.transactionsHash === currentTransactionsHash) {
  ...
  const content = `Gere um relatório com insights sobre as minhas finaças, com dicas e orientações de como melhorar 
    minha vida financeira.As transações estão divididas por pontoe virgula, a estrutura de cada uma e {DATA}-{TIPO}-{VALOR}-{CATEGORIA}. São elas: 
    ${transactions
      .map(
        (transaction) =>
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um especialista em gestão e organização de finaças pessoais. Você ajuda as pessoas a organizarem melhor as suas finaças",
      },
      {
        role: "user",
        content,
  ...
  //   // Salvar o relatório atualizado no banco de dados
  //   if (existingReport) {
  ...
  //       data: {
  //         report,
  //         transactionsHash: currentTransactionsHash,
  //       },
  //     });
  //   } else {
